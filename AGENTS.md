...
- **按键布局标准**：必须坚守任天堂官方硬件标准「左 B 右 A」（左下为 B 键，右上为 A 键），严禁反转操作映射与物理按键。
- **外壳保护与顶点沉降**：原模静态按键网格通过预提取的 `BUTTON_VERTICES` 索引集下陷，严禁改用粗糙的球形距离判断，防止误沉降白色外壳网格导致机身凹陷变形。
- **模拟器生命周期**：切换或弹出卡带时必须调用 `stopEmulator()` 彻底销毁旧实例并重置屏幕贴图，避免 WebAssembly 内存泄漏或音频残留。
