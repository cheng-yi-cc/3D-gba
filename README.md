...
- 📂 **内置 4 款完整可玩的自制游戏**：内置《俄罗斯方块 (Apotris)》、《弗洛拉庄园 (Aunt Flora's Mansion)》、《粉末地牢 (POWDER)》、《雷电球大逃亡 (Voltorb's Escape)》，支持卡带点击即玩与工具栏快捷换卡；亦可将任意本地 `.gba` / `.zip` 游戏文件直接拖入页面游玩。
...
|---|---|---|---|
| **十字方向键 (D-Pad)** | `W` `A` `S` `D` | `↑` `↓` `←` `→` 方向键 | 独立刚性键体绕中心倾斜，黑框固定，支持斜向按压和阻尼归位 |
| **B 键 / A 键** | `J` (B) / `K` (A) | `Z` (B) / `X` (A) | 官方标准「左 B 右 A」实体按键物理微沉 · 键盘左右对应机身左右 |
| **L 键 / R 键** | `U` (L) / `I` (R) | `Q` (L) / `E` (R) | 独立肩键整体绕内侧铰链下压、阻尼回位，机壳固定 |
| **Select / Start** | `Space` / `Enter` | `V` (Select) | 原模型绿键下沉回弹 |
| **视角旋转 / 缩放** | 鼠标左键拖拽 / 滚轮 | 鼠标右键平移视角 | 闲置 4 秒自动恢复优雅低速巡览 · 底部工具栏「🔄 环绕」可手动开关（记忆选择） · 插入卡带游玩时自动暂停 |
...
推送到 GitHub 的 `master` 后，Cloudflare Pages 项目 `gba` 自动执行 `npm run build`，发布 `dist` 到 [gba.chengyi.me](https://gba.chengyi.me/)。构建脚本只发布 `index.html`、`_headers` 和 `assets/`，并检查 Pages 的单文件 25 MiB 和免费方案 20,000 文件上限。主机模型与三个应用模块自动生成内容指纹文件名，HTML 和模块导入同步更新，避免浏览器混用旧脚本与新模型；源码文件名保持不变。
