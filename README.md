...
| GBA 按键 | 键盘推荐按键 | 兼容按键 | 3D 实体与屏幕交互 |
|---|---|---|---|
| **十字方向键 (D-Pad)** | `W` `A` `S` `D` | `↑` `↓` `←` `→` 方向键 | 十字键对应翼片三维物理倾斜下凹 |
| **B 键 / A 键** | `J` (B) / `K` (A) | `Z` (B) / `X` (A) | 主机右侧对应实体按键物理微沉 |
| **L 键 / R 键** | `U` (L) / `I` (R) | `Q` (L) / `E` (R) | 主机上方两侧肩键微动响应 |
| **Select / Start** | `Space` / `Enter` | `V` (Select) | 主机橡胶椭圆按键微动响应 |
