...
- **输入桥接与人体工学映射**：
  - 键盘事件监听器捕获 `keydown` / `keyup`，基于物理键码 `e.code` 消除输入法与大小写干扰：
    ...
    - 核心操作：右手 `J`(A键, 8)、`K`(B键, 0) 及兼容键 `Z`(B)、`X`(A)；
  ...
  - **3D 实体按键交互与拓扑架构**：
    ...
    - 按键拓扑隔离与原顶点按压：不再外加可见按键块，不永久沉降机壳；加载时按真孔位置给原模型按键顶点分类并记录 feather 权重（含肩键 baseColor 颜色锁定，白壳权重为 0），静息零变形，按压时只有原按键几何下沉/倾斜。
    - 四向机械摇杆：十字移动键配备 0.15 rad 四向机械倾角与整键微沉微动，直观呈现机械跷跷板手感。
