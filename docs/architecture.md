...
更新模型时保留具名部件及固定内壁，重新核对真孔位置、铰链、拾取和松键归位；手动检查步骤见 [运维手册](./runbook.md)。发布时 `npm run build` 校验文件体积并将网页资源写入 `dist`，不转换模型或贴图。构建按模型/刚体模块 → 场景 → 应用 → HTML 的依赖顺序生成内容指纹文件名并更新引用，保证浏览器缓存中的旧脚本不会与新模型混用；上面的 `?v=` 仅用于源码预览。
