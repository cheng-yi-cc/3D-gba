...
2. **卡带与模拟器验证**：
   - 点击左侧卡带袋中的预置演示卡带（`test3d.gba`），卡带平滑移动并插入主机的卡槽中。
   - 扬声器触发插卡与微动音效，主机屏幕自发光并出现 GBA 游戏画面。
   - 键盘输入：方向键行走，Z / X 对应 A / B 键，Q / E 对应 L / R 键，V / Enter 对应 Select / Start 键。
3. **导入自定义 ROM**：
   - 将本地 `.gba` 文件直接拖拽至浏览器窗口中，或点击底部工具栏「导入 ROM」。
   - 确认卡带自动生成标签并触发插卡流程。
4. **存档与读档**：
